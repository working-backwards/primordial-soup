...
**Post-discovery exploitation and scale-up.** Every right-tail story in this study ends at the moment of revelation — the tick at which a transformational opportunity is surfaced as a major win. The study counts that discovery; it does not model what happens next: the surge of investment, headcount growth, and organizational reorientation that follows a proven breakthrough in a real company. This boundary is deliberate, and it is what justifies two related simplifications — flat headcount and counting (rather than valuing) major wins.

The justification is governance-invariance: once an opportunity is *proven* transformational, every competent organization scales it, regardless of which governance regime it runs. The governance-relevant variation — the thing this study compares — lives entirely upstream of revelation: which regimes admit enough uncertain attempts, sustain them long enough, and avoid killing them before the truth is visible. Because the post-discovery truncation applies identically to every regime, it cannot bias the cross-regime comparison; the count of surfaced discoveries is a sufficient statistic for the governance question. For the same reason, it does not matter to the study's conclusions whether a surfaced opportunity would have been worth 50% or 150% of the rest of the portfolio combined — what matters is that governance got the organization to the moment of knowing.

Two in-model consequences of this boundary are worth naming. First, flat headcount is also the experimental control that makes regimes comparable at all: identical labor endowments are why outcome differences are attributable to governance rather than to resources. Second, the one resource governance *can* concentrate on an emerging winner is executive attention — within this study, attention concentration is the in-scope representation of "the organization leans in," and the post-revelation capital surge remains out of scope.

**Time discounting.** The simulation assumes infinite patience within the horizon — future value is not discounted relative to present value. This is a deliberate choice aligned with the focus on long-run organizational evolution rather than finite-tenure managerial decisions. Governance regimes optimized for near-term value capture are not modeled. (A discounted-value column in the reporting ledger is planned as a presentation-layer addition — see the 2026-06-10 Repo Improvement Plan, Phase 3.1 — without changing the engine's undiscounted accounting.)
