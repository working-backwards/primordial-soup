#!/usr/bin/env python
"""Model 1 governance comparison campaign.

Runs three stop/intake-posture governance archetypes on the Model 1
configuration and produces a self-contained run bundle with report,
Parquet tables, and figures.

Model 1 is the second rung of the model ladder (per
docs/implementation/2026-06-10 Repo Improvement Plan.md): Model 0 plus
hidden quality with noisy screening signals, active stop rules
(confidence decline, stagnation), and the intake belief floor. It is
the smallest model containing the study's research question —
governance deciding what to start and what to stop under genuine
uncertainty.

Still absent by design: executive attention, team ramp, dependency,
dynamic frontier, staffing response.

The three archetypes share IDENTICAL portfolio mix targets and differ
only in stop/intake posture:
  - Balanced:   intake floor 0.35, confidence decline 0.30, stagnation 15
  - Aggressive: intake floor 0.50, confidence decline 0.40, stagnation 8
  - Patient:    intake floor 0.20, confidence decline 0.08, stagnation 20

Usage:
    python scripts/model1_campaign.py
    python scripts/model1_campaign.py --seeds 5
    python scripts/model1_campaign.py --output-dir results/
"""
...
# SimulationConfiguration with Model 1 parameters baked in.
ARCHETYPES = [
    ("balanced", "Balanced", make_model1_balanced_config),
    ("aggressive", "Aggressive", make_model1_aggressive_config),
    ("patient", "Patient", make_model1_patient_config),
]

# Default seed count. Model 1 runs are small (130 initiatives,
...
def run_model1_campaign(
    seeds: tuple[int, ...],
    output_dir: Path,
) -> Path:
    """Run the Model 1 three-archetype campaign and write a run bundle.
    ...
    logger.info(
        "Model 1 campaign: %d archetypes x %d seeds = %d runs",
        len(ARCHETYPES),
        len(seeds),
        total_runs,
    )
    ...
    for label, display, factory in ARCHETYPES:
        ...
        # Single shared environment ("model1"); conditions vary only by
        # stop/intake posture. Mix targets are identical by design.
        condition_spec = ExperimentalConditionSpec(
            experimental_condition_id=f"model1__{label}",
    ...
    experiment_spec = ExperimentSpec(
        experiment_name="model1_governance_comparison",
        title="Model 1 Governance Comparison",
        description=(
            "Three stop/intake-posture archetypes (Balanced, Aggressive, "
            "Patient) on the Model 1 configuration: Model 0 plus hidden "
            "quality, screening signals, stop rules, and the intake "
            "belief floor. Identical portfolio mix targets across "
            "archetypes; posture is the only treatment dimension."
        ),
        world_seeds=seeds,
        condition_records=tuple(condition_records),
        script_name="scripts/model1_campaign.py",
        study_phase="evaluation",
        baseline_condition_id="model1__balanced",
    )
...
def main() -> None:
    ...
    bundle_path = run_model1_campaign(seeds, args.output_dir)
